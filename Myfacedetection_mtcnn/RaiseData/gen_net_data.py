...
pos_save_dir = "./"+str(net_size)+"/positive"
part_save_dir = "./"+str(net_size)+"/part"
neg_save_dir = "./" + str(net_size) + "/negative"
save_dir = "./"+str(net_size)
...
for annotation in annotations:
    ...
    while neg_num < 50:
        ...
        if np.max(Iou) < 0.3:
            ...
            f2.write(str(net_size)+"/negative/%s"%n_idx + ' 0\n')
    ...
    for box in boxes:
        ...
        for i in range(20):
            ...
            if IoU(crop_box, box_) >= 0.8:
                save_file = os.path.join(pos_save_dir, "%s.jpg"%p_idx)
                f1.write(str(net_size)+"/positive/%s"%p_idx + ' 1 %.2f %.2f %.2f %.2f\n'%(offset_x1, offset_y1, offset_x2, offset_y2))
                cv2.imwrite(save_file, resized_im)
                p_idx += 1
